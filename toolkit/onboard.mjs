...
if (skills.length) {
  ...
  for (const id of skills) {
    ...
    say(`    ${(id + n).padEnd(22)} ${C.dim}${it.repo}${C.off} ${C.y}latest${C.off} ${C.dim}[${it.licence}]${C.off}`);
  }
  say('');
}
if (manual.length) {
...
say(`  into craftkit, always at their latest upstream. They run with full agent`);
say(`  permissions once installed, so what installs is whatever upstream last pushed.${C.off}`);
...
if (skills.length) {
  ...
  for (const id of skills) {
    ...
    try {
      process.stdout.write(plain(`    ${id} ${C.dim}fetching ${it.repo} (latest)…${C.off}`));
